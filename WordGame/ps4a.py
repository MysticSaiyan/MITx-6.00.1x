...
def isValidWord(word, hand, wordList):
    ...
    test = True
    for letter in word:
        if testDict.get(letter,0) > 0:
            test = True
            testDict[letter] = testDict.get(letter) - 1        
        else:
            test = False
            break; 
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    # Game is over (user ent + "\n"ered a '.' or ran out of letters), so tell user the total score
    if word == '.':
